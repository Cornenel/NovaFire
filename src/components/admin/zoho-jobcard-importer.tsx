...
function SummaryCards({ preview }: { preview: NonNullable<ZohoImportActionState["preview"]> }) {
  ...
  const cards = [
    ...
    ["Quote groups required", validation.quotes_required],
...
function ResultPanel({ result }: { result: NonNullable<ZohoImportActionState["result"]> }) {
  ...
  const rows = [
    ...
    ["Annual services completed", validation.annual_services_completed],
    ["Pressure tests required", validation.pressure_tests_required],
    ["Quote groups required (CSV)", validation.quotes_required],
    ["Quote groups created", validation.quotes_required_records_created],
